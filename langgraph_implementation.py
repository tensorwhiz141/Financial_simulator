...
def get_model_for_task(task_name: str, context_size: int = 0) -> str:
    """
    Select appropriate model based on task complexity and context size.
    Use smaller models for simple tasks or when context is large.
    """
    # If context is very large, use smaller model
    if context_size > 8000:  # Rough character count
        return "groq/llama3-8b-8192"  # Smaller, faster model

    # Task-specific model selection
    if task_name in ["behavior_tracker", "karma_tracker"]:
        return "groq/llama3-8b-8192"  # Simpler tasks can use smaller model
    elif task_name in ["financial_strategy", "goal_tracker"]:
        return "groq/llama3-70b-8192"  # Complex analysis needs larger model
    else:
        return "groq/llama3-8b-8192"  # Default to smaller model for efficiency
        ...
        try:
            ...
            if "rate_limit_exceeded" in error_str or "Request too large" in error_str:
                if attempt < max_retries - 1:
                    ...
                    print(f"❌ Max retries reached for rate limit error: {error_str}")
...
def get_llm(model_name="groq/llama3-70b-8192"):
    ...
    if model_name.startswith("groq/"):
        if groq_api_key:
            # Automatically use smaller models to avoid rate limits
            model_fallbacks = {
            ...
            if actual_model in model_fallbacks:
                actual_model = model_fallbacks[actual_model]
                print(f"🔄 Using smaller model {actual_model} to avoid rate limits")
            ...
            return ChatGroq(
                model_name="llama3-8b-8192",  # Use smaller model
                temperature=0.2,
                max_tokens=512
            )
        ...
        # Default to Groq with smaller model
        if groq_api_key:
            return ChatGroq(
                groq_api_key=groq_api_key,
                model_name="llama3-8b-8192",  # Use smaller model by default
                temperature=0.2,
                max_tokens=512  # Reduced to avoid rate limits
            )
        else:
