...
logger = configure_logging("financial_simulator")
...
try:
    ...
    if key:
        agentops.init(api_key=key, default_tags=['langgraph'])
        logger.info("✅ AgentOps initialized successfully")
    else:
        logger.info("ℹ️ AGENTOPS_API_KEY not set; skipping AgentOps initialization")
except Exception as e:
    logger.warning(f"⚠️ AgentOps initialization failed: {e}")
    logger.warning("⚠️ Continuing without AgentOps...")
app = FastAPI()
