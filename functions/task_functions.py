...
def truncate_context_for_api_limits(context: str, max_tokens: int = 4000) -> str:
    """
    Truncate context to stay within API token limits.
    ...
    truncated += f"\n\n[Context truncated to stay within API limits. Original length: {len(context)} chars, truncated to: {len(truncated)} chars]"
    return truncated

def limit_historical_data(data: list, max_months: int = 3) -> list:
    """
    Limit historical data to the most recent months to reduce context size.
    """
    if len(data) <= max_months:
        return data

    # Sort by month and take the most recent ones
    sorted_data = sorted(data, key=lambda x: x.get('month', 0), reverse=True)
    return sorted_data[:max_months]
...
def build_simulated_cashflow_context(month_number, user_id, flag=True):
    ...
    try:
        ...
        # Limit historical data to prevent API rate limits
        data = limit_historical_data(data, max_months=3)
        ...
        # Truncate context to stay within API limits
        return truncate_context_for_api_limits(context, max_tokens=4000)
    ...
    try:
        ...
        for entry in data:
            ...
            if month is not None and month < month_number:
                ...
                # Handle recommendations that might be strings or dicts
                processed_recommendations = []
                for rec in recommendations:
                    if isinstance(rec, dict):
                        # If it's a dict, try to extract meaningful text
                        if 'text' in rec:
                            processed_recommendations.append(rec['text'])
                        elif 'recommendation' in rec:
                            processed_recommendations.append(rec['recommendation'])
                        elif 'description' in rec:
                            processed_recommendations.append(rec['description'])
                        else:
                ...
                all_recommendations.extend(processed_recommendations)
                rec_str = "; ".join(processed_recommendations) if processed_recommendations else "No recommendations provided"
